...
const Product = () => {
  ...
  const fetchProductData = async () => {
    if (!products || !productId) return; // Ensure products and productId are available
  ...
  useEffect(() => {
    fetchProductData();
  }, [products, productId]); // Run effect when products or productId changes

  useEffect(() => {
    if (productData) {
      console.log("Product Data:", productData); // Log productData only when it's available
    }
  }, [productData]); // Log productData when it updates
    ...
    <div className="pt-10 border-t-2 border-gray-300 transition-opacity ease-in duration-500 opacity-100">
      <div className="flex flex-col md:flex-row gap-12">
        ...
        <div className="flex-1 px-6">
          ...
          <p className="text-md mt-6 text-gray-500">{productData.description}</p>
          ...
          <button
            className="cursor-pointer text-white bg-black text-sm px-8 py-3 mt-6"
      ...
      <div className="mt-24">
        <div className="flex flex-row text-sm">
          ...
          <h1 className="border-2 border-gray-200 border-b-0 px-3 py-2">
            Reviews(122)
          </h1>
        </div>
        <div className="border-2 border-r-0 border-gray-200 text-sm px-6 py-6 text-gray-500">
          <p className="mb-2">
